...
LOG_FILENAME = '/var/log/squid3/squid-redirector.log'
...
#      logging.warning('Impossible de recuperer l\'origine du client depuis la chaine '+line_squid)
    ...
    logging.debug(client_input + ' Reception d\'une URL par : ' + url_input)
        ...
        if re.search('http://' + redirectors[i][0],url_input):
          ...
          if redirectors[i][1] == "IGNORE" :
            ...
            try:
              ...
              logging.debug(client_input + ' Ce fichier est en cache : ' + url_output)
              found = 1
              break
            except:
    ...
    if ignore == 1 :
      ...
      if found == 0 :
        logging.debug(client_input + ' Aucun depot ne contient cette URL ')
        print url_input #+ '\n'
      else :
